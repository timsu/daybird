defmodule Sequence.OAuthTokensTest do
  ...
  describe "oauth_tokens" do
    ...
    test "create_oauth_token/1 with valid data creates a oauth_token" do
      valid_attrs = %{access: "some access", user_id: 1, expires_at: ~N[2022-10-19 23:40:00], meta: %{}, name: "some name", refresh: "some refresh"}

      assert {:ok, %OAuthToken{} = oauth_token} = OAuthTokens.create_oauth_token(valid_attrs)
      assert oauth_token.access == "some access"
      assert oauth_token.expires_at == ~N[2022-10-19 23:40:00]
    ...
    test "update_oauth_token/2 with valid data updates the oauth_token" do
      oauth_token = oauth_token_fixture()
      update_attrs = %{access: "some updated access", deleted_at: ~N[2022-10-20 23:40:00], expires_at: ~N[2022-10-20 23:40:00], meta: %{}, name: "some updated name", refresh: "some updated refresh"}
      ...
      assert oauth_token.deleted_at == ~N[2022-10-20 23:40:00]
      assert oauth_token.expires_at == ~N[2022-10-20 23:40:00]
      assert oauth_token.meta == %{}
