...
// FROM https://github.com/danfuzz/bayou/blob/main/local-modules/%40bayou/quill-util/QuillUtil.js
 ...
 */
export function quillContextForPixelPosition(quillInstance: Quill, x: number, y: number) {
  if (!quillInstance) {
    return POSITION_NOT_FOUND
  }

  const container: HTMLDivElement = (quillInstance as any).container
  ...
  // find the offset.
  if (!blotOrQuill || blotOrQuill instanceof Quill) {
    return POSITION_NOT_FOUND
  }
